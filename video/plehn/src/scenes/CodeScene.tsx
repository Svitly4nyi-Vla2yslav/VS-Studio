...
export const CodeScene: React.FC = () => {
  ...
  const handoff=interpolate(frame,[330,520],[0,1],{extrapolateLeft:'clamp',extrapolateRight:'clamp'});
  const pushIn=interpolate(frame,[0,sceneFrames.code],[1,1.035]);
  return <SceneFrame duration={sceneFrames.code} label="06 · Code als Quelle" accent={palette.blue}>
    <div style={{display:'grid',gridTemplateColumns:'.88fr 1.12fr',gap:72,alignItems:'center'}}>
      ...
      <div style={{position:'relative',height:610}}>
        ...
        <div style={{position:'absolute',right:0,bottom:0,width:600,height:360,borderRadius:23,border:`1px solid ${palette.gold}88`,overflow:'hidden',background:'radial-gradient(circle at 70% 20%,rgba(239,179,62,.22),transparent 35%),#111721',opacity:handoff,transform:`translateY(${(1-handoff)*55}px) scale(${(.9+handoff*.1)*pushIn})`,boxShadow:'0 34px 90px rgba(0,0,0,.5)'}}><div style={{padding:38}}><div style={{color:palette.cyan,fontSize:16,letterSpacing:3}}>RENDERED EXPERIENCE</div><div style={{fontSize:48,fontWeight:760,lineHeight:1.05,marginTop:25}}>Ich zeige es<br/><span style={{color:palette.gold}}>lieber.</span></div><div style={{marginTop:35,display:'inline-flex',padding:'13px 19px',borderRadius:10,background:palette.gold,color:palette.bg,fontWeight:700}}>Projekt ansehen</div></div></div>
      </div>
    </div>
  </SceneFrame>;
};
